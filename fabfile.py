...
def create_deploy_directories(instance):
    ...
    sudo("mkdir -p %s/virtualenv %s/htdocs %s/log" % ( basedir, basedir, basedir))
    sudo("chown -R www-data:www-data %s" % basedir)
    
def create_virtualenv(instance):
    dirname = "/srv/www/%s" % instance
    cmd = "virtualenv /srv/www/%s/virtualenv" % instance
...
def install_postgis():
    # from http://trac.osgeo.org/postgis/wiki/UsersWikiPostGIS21UbuntuPGSQL93Apt
    sudo("sh -c 'echo \"deb http://apt.postgresql.org/pub/repos/apt/ precise-pgdg main\" >> /etc/apt/sources.list")
    run("wget --quiet -O - http://apt.postgresql.org/pub/repos/apt/ACCC4CF8.asc | sudo apt-key add -")
    update_packages()
    sudo("sudo apt-get install Postgresql-9.3-postgis postgresql-contrib")
...
def create_databases():
    ...
    run('su -s /bin/bash -c "%s" postgres' % cmd)
    cmd = "psql -h localhost -U osm -d maproulette -c 'CREATE EXTENSION postgis'"
    run('su -s /bin/bash -c "%s" postgres' % cmd)
    cmd = "psql -h localhost -U osm -d maproulette_test -c 'CREATE EXTENSION postgis'"
    run('su -s /bin/bash -c "%s" postgres' % cmd)
